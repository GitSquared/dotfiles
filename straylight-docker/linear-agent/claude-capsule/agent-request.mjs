  ...
  if (!disposition) {
    ...
    return {
      ...
      reason: context.signaledSinceLastTransition
        ? "A Signal alone never ends a turn - it only posts a comment and continues. To actually stop, call finish_work with status: answered if the request was purely conversational and you already replied directly (nothing to approve, so no QA card or invented evidence needed); request blocking Steering when an answer is required; or request QA with evidence when a deliverable is ready for human approval. If you believe there is genuinely nothing new to do on real delegated work, that still means requesting QA again with the still-valid (or fresh) evidence, not stopping."
...
export function createStraylightTools(context) {
  ...
  const tools = [
    ...
    tool(
      "finish_work",
      "Declare how this turn ends outside Steering/QA. Use answered when the request was purely conversational - a question, clarification, or discussion with nothing to approve - and you already replied directly (a comment, Document-thread reply, or linear_activity response); no evidence or approval was ever needed for that, so don't route it through QA just to close the turn. Use blocked_external only for a non-human dependency with a concrete retry condition, and deferred only when the authoritative request permits postponement. Delegated work that produces a deliverable - a code change, an artifact, a decision to approve - still must end through QA; the agent may not declare that kind complete on its own.",
      {
        status: z.enum(["answered", "blocked_external", "deferred"]),
        reason: z.string().min(1).max(2_000),
        nextAction: z.string().min(1).max(1_000).optional().describe("Required for blocked_external/deferred (the concrete retry condition or resume point). Omit for answered - there is nothing next."),
      },
      async (request) => {
    ...
    tool(
      "manage_linear",
      "Get, create, update, list, link, or unlink native Linear issues, subissues, projects, Documents, review comments, and relationships through the credential broker - which operations are valid depends on the resource: issue and project support get, create, update, or delete; document supports list, get, create, update, or delete; comment supports list, get, create, reply, update, resolve, unresolve, or delete; relation supports list, create (alias link), or delete (alias unlink); subissue supports list, create, link, or unlink. Creating a subissue never accepts assigneeId or delegateId - it always starts unassigned; if a human genuinely needs to own it, assign that afterward with a separate issue update, not at creation time. Any other operation for a resource is rejected. Document create posts directly on the current issue, no project required, and its id field means something different per operation: on create, id names the issue to attach the Document to (defaults to the current issue), not the Document itself; on get/update/delete, id is the Document's own id; list ignores id and uses parentId instead (also defaulting to the current issue) to say which issue's Documents to enumerate. Comment list and create with no parentId target the current issue's own comments; pass a Document id as parentId to list or comment on a Document instead, or reply within an existing thread by id. If the session context shows more than one comment thread on the issue, reply within the one the request is actually about rather than starting an unrelated new comment.",
      {
