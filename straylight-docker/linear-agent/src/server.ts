  ...
  async function route(request: Request, server?: ControllerServer): Promise<Response> {
    ...
    if (method === "GET" && matches(url.pathname, "/tools/auth")) {
      return text(200, [
        ...
        "",
        "Git commit identity and signing key (optional, once GitHub auth above works):",
        "",
        "  docker compose run --rm --no-deps --entrypoint git linear-agent-runner config --global user.name \"Your Name\"",
        "  docker compose run --rm --no-deps --entrypoint git linear-agent-runner config --global user.email \"you@example.com\"",
        "",
        "  # Optional commit signing with an SSH key - openssh-client is already in the image, no GPG needed:",
        "  ssh-keygen -t ed25519 -C \"straylight commit signing\" -N \"\" -f linear-agent/tool-profile/signing/id_ed25519",
        "  docker compose run --rm --no-deps --entrypoint git linear-agent-runner config --global gpg.format ssh",
        "  docker compose run --rm --no-deps --entrypoint git linear-agent-runner config --global user.signingkey /tool-profile/signing/id_ed25519",
        "  docker compose run --rm --no-deps --entrypoint git linear-agent-runner config --global commit.gpgsign true",
        "  # Add the printed .pub key to GitHub as a *Signing Key* (not just an Authentication Key): https://github.com/settings/keys",
        "",
        ...
        "",
        "The profile is mounted at /tool-profile. GH_CONFIG_DIR and GIT_CONFIG_GLOBAL point there, so authentication, commit identity, and signing keys survive task containers and image rebuilds.",
